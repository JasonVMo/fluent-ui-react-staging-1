...
export const NotchedSlider = compose<INotchedSliderType>({
  ...
  usePrepareProps: (
    props: INotchedSliderProps,
    useStyling: IUseComposeStyling<INotchedSliderType>
  ) => {
    const forwarded = { root: props };
    const vertical = props.vertical;
    const styleProps = useStyling(props);
    const notches = props.hasOwnProperty("notches")
      ? props.notches
      : (styleProps.tokens && styleProps.tokens.notches) || 0;
    ...
    return {
      slotProps: mergeSettings<INotchedSliderType["slotProps"]>(
        styleProps,
        forwarded
      ),
      state: { vertical, notchPos }
    };
  },
  // eslint-disable-next-line react/display-name
  render: (
    Slots: IComposeSlots<INotchedSliderType>,
    renderData: IComposeRenderData<INotchedSliderType>,
